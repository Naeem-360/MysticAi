...
class HoloGUI(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle("Holographic AI Assistant")
        self.setGeometry(200, 200, 800, 600) 
        self.setStyleSheet("background-color: #0a0a0a; border: 1px solid cyan;")
        ...
        self.title.setStyleSheet("color: #00ffff; text-shadow: 0 0 5px cyan;")
        layout.addWidget(self.title)
        self.output = QTextEdit(self)
        self.output.setReadOnly(True)
        self.output.setStyleSheet("color: #00ffff; background-color: rgba(0, 0, 0, 0.7); border: none;")
        self.output.setFont(QFont("Arial", 16))
        layout.addWidget(self.output)
        self.input_field = QLineEdit(self)
        self.input_field.setStyleSheet("color: cyan; background-color: rgba(0, 0, 0, 0.7); border: 2px solid cyan;")
        self.input_field.setFont(QFont("Arial", 14))
        self.input_field.returnPressed.connect(self.process_input)
        layout.addWidget(self.input_field)

        self.toggle_btn = QPushButton("Toggle Voice/Text Mode", self)
        self.toggle_btn.setStyleSheet("background-color: transparent; border: 2px solid cyan; color: cyan; padding: 10px;")
        self.toggle_btn.clicked.connect(self.toggle_mode)
        layout.addWidget(self.toggle_btn)
        self.stop_btn = QPushButton("Stop Talking", self)
        self.stop_btn.setStyleSheet("background-color: transparent; border: 2px solid cyan; color: cyan; padding: 10px;")
    ...
    def paintEvent(self, event):
        ...
        pen = QPen(QColor(0, 255, 255, 100), 2)  
        painter.setPen(pen)
        ...
        outer_pen = QPen(QColor(0, 255, 255, 50), 1)
    ...
    def flicker_effect(self):
        ...
        if "opacity: 0.8" in current_style:
            self.title.setStyleSheet("color: #00ffff; text-shadow: 0 0 5px cyan; opacity: 1;")
        else:
            self.title.setStyleSheet("color: #00ffff; text-shadow: 0 0 5px cyan; opacity: 0.8;")
    ...
    def process_voice_input(self, text):
        if text:
            ...
            talk("Sorry, I didn’t catch that.", self)
        if current_mode == "voice":
            self.start_voice_input()
